...
<%-- 		
			...
			<li role="presentation" class="divider" style="margin:0px!important;"></li>
			<li><a href="<%=request.getContextPath()%>/user/userInfo" style="text-align:center;"><span class="glyphicon glyphicon-user"></span>${user.username}</a></li>
		</ul>
	...
	<script>
		$('#calendar').datepicker({});
		var ERROR_MSG = "服务器错误，请及时联系系统管理员!";
		!function($) {
			$(document)
					.on(
							"click",
		...
		function productStr(variable){
			return "<select class='form-control skuId combobox_"+variable+"'  onChange='productChange(this)'>"+
			"<option value='-1'>请选择</option>"+
			"<c:if test='${not empty productList}'>"+
				"<c:forEach var='product' items='${productList}'>"+
					"<option value='${product.code}' productName='${product.name}' base_unit='${product.erpUnit}' base_unit_code='${product.erpUnitCode}' min_unit='${product.erpMinUnit}' min_unit_code='${product.erpMinUnitCode}' unit_price='${product.unitPrice}'  unit_quantity='${product.unitQuantity}'>${product.code}-${product.name}</option>"+
				"</c:forEach>"+
			"</c:if>"+
			"</select>";
		}
		
		function terminalStr(variable){
			return "<select class='form-control terminalId combobox_"+variable+"'>"+
			"<option value='-1'>请选择</option>"+
			"<c:if test='${not empty terminalList}'>"+
				"<c:forEach var='terminal' items='${terminalList}'>"+
					"<option value='${terminal.terminalId}'>${terminal.terminalId}-${terminal.terminalName}</option>"+
				"</c:forEach>"+
			"</c:if>"+
			"</select>"
		}
