	...
	<div class="col-lg-12">
		<div class="panel panel-default" style="padding-bottom: 5px;">
			<div class="panel-body">
				<span class="glyphicon glyphicon-edit" aria-hidden="true"></span>销售计划新增
				...
				<br /> <br />
					...
					<input type='hidden' name='terminalId' id="terminalId"
						value='${terminalId}' />
					<input type='hidden' name='salePlanId' id="salePlanId"
						value='${salePlanId}' />
...
<script type="text/javascript">
	...
	function countPrice(){
		...
		$.each($("#sku_tb").find("tr"),function(i,mtr){
			if(i!=0){
				console.log($(mtr).eq(0).find(".totalPrice").text());
	...
	function iterateTable(){
		...
		$.each($("#sku_tb").find("tr"),function(i,mtr){
			if(i!=0){
				...
				var baseQuantity=$(mtr).eq(0).find(".quantity").val();
					if(skuIdVal=="-1"||skuIdVal==""||skuIdVal==undefined||baseQuantity==""||isNaN(baseQuantity)){
						flag = false;
					}
					var planItem = {
		...
		if(flag){
			...
			$.ajax({
				url : '<%=request.getContextPath()%>/terminal/addSalePlanItem',
				dataType: "json", 
