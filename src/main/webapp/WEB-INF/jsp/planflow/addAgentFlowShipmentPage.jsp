...
<script type="text/javascript">
	...
	function iterateTable(){
		...
		$.each($("#sku_tb").find("tr"), function(i, mtr) {
			if (i != 0) {
				...
				var skuIdVal = $(mtr).eq(0).find(".skuId").find(
						"option:selected").val();
				var terminalIdVal = $(mtr).eq(0).find(".terminalId").find(
						"option:selected").val();
				...
				var agentFlowShipmentItem = {
					...
					terminalId : $(mtr).eq(0).find(".terminalId").find(
							"option:selected").val()
				}
		...
		if(flag){
			...
			$.ajax({
				url : '<%=request.getContextPath()%>/agent/addAgentFlow',
				dataType: "json", 
